...
# ── Release signing ────────────────────────────────────────────────────────
# Build a signed release APK (not debug)
android.debug = False

# Keystore generated by OpenSSL (PKCS12 format, compatible with apksigner)
android.keystore = release.keystore
android.keyalias = oussama_sat

# Credentials — loaded from environment variables set in .keystore.env
# Before building, run: export $(grep -v '^#' .keystore.env | xargs)
android.keystore_passwd = %(environ.get('KEYSTORE_PASSWORD', ''))s
android.keyalias_passwd = %(environ.get('KEY_PASSWORD', ''))s

# Output: release APK (unsigned by buildozer; sign_release.sh finishes it)
android.release_artifact = apk

# ── Runtime behaviour ──────────────────────────────────────────────────────
# Keep CPU awake during long flash operations
